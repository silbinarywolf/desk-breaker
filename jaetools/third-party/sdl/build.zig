...
pub fn build(b: *std.Build) void {
    ...
    {
        const sdl_include_path = sdl3_dep.path("include");
        b.addNamedLazyPath("include_path", sdl_include_path);
        _ = blk: {
    ...
    const build_config_h: *std.Build.Step.ConfigHeader = build_config_h: {
        ...
        break :build_config_h b.addConfigHeader(.{
            ...
            .HAVE_PPOLL = linux, // Android/iOS Branch: 'or macos or emscripten or ios', <- Unsure of validity for iOS
            ...
            .SDL_THREAD_PTHREAD_RECURSIVE_MUTEX_NP = false, // Android/Ios branch: android <- Likely not needed as "SDL_THREAD_PTHREAD_RECURSIVE_MUTEX" is true
            ...
            .SDL_VIDEO_RENDER_GPU = windows or linux or macos or android or ios,
    ...
    const sdl_lib = b.addLibrary(.{
        ...
        .use_llvm = if (emscripten or android) true else null,
    });
    sdl_lib.lto = lto;
    ...
    var sdl_c_flags_buf: [common_c_flags.len + 4][]const u8 = undefined;
    ...
    switch (sdl_lib.linkage.?) {
        ...
        .dynamic => {
            ...
            sdl_uclibc_mod.addIncludePath(sdl3_dep.path("include"));
    ...
    if (windows) {
        ...
        if (sdl_lib.linkage.? == .dynamic) {
            sdl_mod.addWin32ResourceFile(.{ .file = sdl3_dep.path("src/core/windows/version.rc") });
        }
    }
    if (linux) {
        sdl_mod.addCSourceFiles(.{
    ...
    sdl_lib.installHeadersDirectory(sdl3_dep.path("include/SDL3"), "SDL3", .{
        .exclude_extensions = &.{
            "SDL_revision.h",
            "SDL_test.h",
            "SDL_test_assert.h",
    ...
    sdl_test_mod.addIncludePath(sdl3_dep.path("include"));
    if (system_include_path) |path| {
    ...
    sdl_test_lib.installHeadersDirectory(sdl3_dep.path("include/SDL3"), "SDL3", .{
        .include_extensions = &.{
            "SDL_test.h",
            "SDL_test_assert.h",
            "SDL_test_common.h",
