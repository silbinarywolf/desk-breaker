//! TODO: Remove this file as it shouldn't be needed anymore
...
pub fn start(app: *App, init: std.process.Init) !void {
    ...
    global_c_allocator = try GlobalCAllocator.init(allocator, io);
    ...
    user_settings.* = UserConfig.load(allocator, io) catch |err| switch (err) {
    ...
    app.* = try App.init(allocator, io, user_settings);
    errdefer app.deinit();
