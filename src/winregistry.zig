...
pub const RegistryWtf8 = struct {
    ...
    pub fn deleteValue(reg: RegistryWtf8, value_name: []const u8) !void {
        ...
        try registry_wtf16le.deleteValue(value_name_wtf16le);
    }

    pub fn setString(reg: RegistryWtf8, value_name: []const u8, value: []const u8) error{ OutOfMemory, NoAccess, ValueNameNotFound, NotAString, StringNotSet }!void {
        const value_name_wtf16le: [:0]const u16 = value_name_wtf16le: {
            var value_name_wtf16le_buf: [RegistryWtf16Le.value_name_max_len]u16 = undefined;
            const value_name_wtf16le_len: usize = std.unicode.wtf8ToWtf16Le(value_name_wtf16le_buf[0..], value_name) catch unreachable;
            value_name_wtf16le_buf[value_name_wtf16le_len] = 0;
        ...
        const value_wtf16le: [:0]const u16 = value_wtf16le: {
            var buf: [RegistryWtf16Le.value_name_max_len]u16 = undefined;
            const len: usize = std.unicode.wtf8ToWtf16Le(buf[0..], value) catch unreachable;
            buf[len] = 0;
            break :value_wtf16le buf[0..len :0];
        };

        const registry_wtf16le: RegistryWtf16Le = .{ .key = reg.key };
        try registry_wtf16le.setString(value_name_wtf16le, value_wtf16le);
    }
...
const RegistryWtf16Le = struct {
    ...
    fn deleteValue(reg: RegistryWtf16Le, value_name_wtf16le: [:0]const u16) error{ StringNotSet, ValueNameNotFound, NoAccess }!void {
        const return_code_int = RegDeleteValueW(reg.key, value_name_wtf16le);
        ...
        switch (return_code) {
            ...
            .NOACCESS => return error.NoAccess,
            else => {
                // std.debug.print("error code: {}\n", .{return_code});
                return error.StringNotSet;
            },
        }
    }

    fn setString(reg: RegistryWtf16Le, value_name_wtf16le: [:0]const u16, value_wtf16le: [:0]const u16) error{ NoAccess, ValueNameNotFound, NotAString, StringNotSet }!void {
        const value_wtf16le_buf_len: DWORD = @intCast((value_wtf16le.len + 1) * @sizeOf(u16)); // in bytes including null terminators
        const return_code_int = RegSetValueExW(
            reg.key,
            value_name_wtf16le,
            0, // reserved
            windows.REG.SZ,
