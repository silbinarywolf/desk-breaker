...
const Gcc = @import("../gccbuild/Gcc.zig");
...
pub fn buildPBP(tools: *Tools, artifact: *Step.Compile) void {
    const b = tools.b;
    ...
    const psp_elf: std.Build.LazyPath = pspcompileblk: {
        ...
        // Setup linker script to use
        //
        // -Wl,-q,-T$(PSPSDK)/lib/linkfile.prx -nostartfiles -Wl,-zmax-page-size=128
        artifact.setLinkerScript(pspdev_path.path(b, "psp/sdk/lib/linkfile.prx"));
        ...
        //
        // Explored this to resolve errors like:
        // "relocation truncated to fit: R_MIPS_GPREL16 against `wuffs_base__error__bad_receiver'"
        // artifact.lto = .full;
        ...
        const gcc_artifact = gcc.convertArtifactExe(artifact);
        const run = gcc_artifact.run;

        // NOTE(jae): 2026-01-17
        // We used to use 'third-party/psp/src/psp/crt0_prx.zig' but we no longer maintain it ourselves
        // run.addArg("-nostartfiles");

        run.addArg("-Wl,-zmax-page-size=128");
        ...
        // *startfile:
        // crt0_prx%O%s crti%O%s crtbegin%O%s
        run.addPrefixedFileArg("-specs=", pspdev_path.path(b, "psp/sdk/lib/prxspecs"));
    ...
    const psp_outfile = fixup_imports_cmd.addOutputFileArg("psp-fixed-imports.elf");
    ...
    // Add each PSP related build file to the 'zig-out/psp'
